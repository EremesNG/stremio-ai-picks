...
const metaHandler = async function (args) {
  ...
  try {
    ...
    if (config) {
      ...
      if (/2\.5|[3-9]\./i.test(modelName) && !/-lite/i.test(modelName)) {
        genaiConfig.thinkingConfig = { thinkingBudget: 1024 };
      }
      ...
      try {
        aiResult = await withRetry(
          async () => {
            ...
            return await ai.models.generateContent({
              ...
              if (error.httpStatus) error.status = error.httpStatus;
              throw error;
            }
          },
          {
        ...
        logger.warn("Gemini similar content call failed", {
          message: error.message,
